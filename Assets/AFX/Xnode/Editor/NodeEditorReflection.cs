...
namespace XNodeEditor {
    ...
    public static class NodeEditorReflection {
        ...
        [NonSerialized] private static Dictionary<Type, Color> nodeAccentTint;
        [NonSerialized] private static Dictionary<Type, int> nodeWidth;
        [NonSerialized] private static Dictionary<Type, string> nodeTitle;
        ...
        /// <summary> Custom node tint colors defined with [NodeAccentColor(Color)] </summary>
        public static bool TryGetAttributeAccentTint(this Type nodeType, out Color tint)
        {
            if (nodeAccentTint == null)
            {
                CacheAttributes<Color, XNode.Node.NodeAccentTintAttribute>(ref nodeAccentTint, x => x.color);
            }
            return nodeAccentTint.TryGetValue(nodeType, out tint);
        }
        ...
        /// <summary> Get custom node title defined with [NodeTitle(title)] </summary>
        public static bool TryGetAttributeTitle(this Type nodeType, out string title)
        {
            if (nodeTitle == null)
            {
                CacheAttributes<string, XNode.Node.NodeTitleAttribute>(ref nodeTitle, x => x.title);
            }

            return nodeTitle.TryGetValue(nodeType, out title);
        }
