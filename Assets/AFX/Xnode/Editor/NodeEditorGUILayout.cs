...
namespace XNodeEditor {
    ...
    public static class NodeEditorGUILayout {
        ...
        private static float portSize = 10;
        private static float portOffsetY = 4;
        private static float portOffsetX = 5;
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            else
            {
                ...
                if (port.direction == XNode.NodePort.IO.Input)
                {
                    ...
                    //set position
                    rect.position = rect.position - new Vector2(portSize + portOffsetX, -(spacePadding + portOffsetY));
                ...
                else if (port.direction == XNode.NodePort.IO.Output)
                {
                    ...
                    //set position
                    rect.position = rect.position + new Vector2(rect.width + portOffsetX, (spacePadding + portOffsetY));
                }

                //rect.size = new Vector2(16, 16);
                rect.size = new Vector2(portSize, portSize);
               ...
               DrawPortHandleWithOverride(port, rect, backgroundColor, col);
        ...
        public static void PortField(GUIContent label, XNode.NodePort port, params GUILayoutOption[] options) {
            ...
            if (port.direction == XNode.NodePort.IO.Input) {
                ...
                position = rect.position - new Vector2(portSize + portOffsetX, -portOffsetY);
            }
            // If property is an output, display a text label and put a port handle on the right side
            else if (port.direction == XNode.NodePort.IO.Output) {
                // Display a label
                ...
                Rect rect = GUILayoutUtility.GetLastRect();
                position = rect.position + new Vector2(rect.width + portOffsetX, (portOffsetY));
            }
        ...
        public static void PortField(Vector2 position, XNode.NodePort port) {
            ...
            Rect rect = new Rect(position, new Vector2(portSize, portSize));
        ...
        public static void AddPortField(XNode.NodePort port) {
            ...
            if (port.direction == XNode.NodePort.IO.Input) {
                rect = GUILayoutUtility.GetLastRect();
                rect.position = rect.position - new Vector2(portSize + portOffsetX, -(portOffsetY));
                // If property is an output, display a text label and put a port handle on the right side
            } else if (port.direction == XNode.NodePort.IO.Output) {
                rect = GUILayoutUtility.GetLastRect();
                rect.position = rect.position + new Vector2(rect.width + portOffsetX, (portOffsetY));
            }
        ...
        private static void DrawPortHandleWithOverride(NodePort port, Rect rect, Color backgroundColor, Color col)
        {
            Texture texture;
            if (NodeEditorResources.TextureOverrides.TryGetValue(port.ValueType.Name, out texture))
            {
                DrawPortHandle(rect, backgroundColor, col, texture);
            }
            else
            {
                if (port.IsConnected)
                {
                    DrawPortHandle(rect, backgroundColor, col, NodeEditorResources.dot);
                }
                else
                {
                    DrawPortHandle(rect, backgroundColor, col, NodeEditorResources.dotOuter);
                }
            }
        }
        ...
        public static void DrawPortHandle(Rect rect, Color backgroundColor, Color typeColor, Texture texture)
        {
            Color col = GUI.color;
            GUI.color = backgroundColor;
            //GUI.DrawTexture(rect, NodeEditorResources.dotOuter);
