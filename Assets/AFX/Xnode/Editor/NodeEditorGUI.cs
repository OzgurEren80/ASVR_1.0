            ...
            switch (path) {
                ...
                case NoodlePath.ShaderLab:
                    ...
                    for (int i = 0; i < length - 1; i++) {
                        ...
                        Vector2 prev_point = point_a;
                        // Approximately one segment per 5 pixels
                        float segments = Vector2.Distance(point_a, point_b) / 6 * zoom ;
                        segments = Math.Max(segments, 1);

                        int draw = 0;
                        for (int j = 0; j <= segments + 5; j++) {
                            draw++;
                            float t = j / segments;
                            Vector2 lerp = Vector2.Lerp(point_a, point_b, t);
                            if (draw > 0) {
                ...
                foreach (XNode.NodePort output in node.Outputs) {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++) {
                        ...
                        for (int i = 0; i < reroutePoints.Count; i++) {
                            ...
                            Rect rect = new Rect(reroutePoints[i], new Vector2(portSize, portSize));
                            rect.position = new Vector2(rect.position.x  - (portSize * 0.5f), rect.position.y  - (portSize * 0.5f));
